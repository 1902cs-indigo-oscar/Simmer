...
const recipesList = [
  ...
  // "https://www.foodnetwork.com/recipes/photos/summer-weeknight-dinners",
  // "https://www.foodnetwork.com/recipes/giada-de-laurentiis/penne-with-sun-dried-tomato-pesto-recipe-1942250",
...
let createRecipeObj = recipe =>
  new Promise((resolve, reject) => {
    console.log("recipe in promise", recipe);
    let urlTail = recipe.split("www.")[1];
    const urlBase = urlTail.split(".com")[0];
    let recipeObj = scraperObj[urlBase](recipe).then(obj =>
      console.log("resolve at end of createRecipeObj", obj)
    );

    resolve(recipeObj);
  });

const createRecipeArr = async recipes => {
  console.log("recipes passed into createRecipeArr", recipes);
  let recipeArr = await recipes.map(async recipe => {
    let recipeObj = await createRecipeObj(recipe).then(obj => obj);
    return recipeObj;
  });
  Promise.all(recipesList).then(arr => {
    console.log("recipeArr at end of createRecipeArr", recipeArr);
    return recipeArr;
  });
};
...
async function seed() {
  try {
    ...
    await Promise.map(recipesList, recipe => {
      ...
      .then(async recipeArr => {
        console.log("recipeArr after then", recipeArr)
        let createdRecipes = await Promise.map(recipeArr, async recipe => {
          console.log("recipe in Promise.all", recipe)
          ...
          await createdArticle.addUser(users[recipesList.indexOf(recipe) % 5]);
    ...
    //       await createdArticle.addIngredient(newIngred);
    //     });
    //     await createdArticle.addUser(users[recipes.indexOf(recipe) % 5]);
    //   } catch (error) {
    //     console.log(error);
    //   }
    // });
